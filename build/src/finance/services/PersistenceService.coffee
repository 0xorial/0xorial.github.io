app.service 'PersistenceService', (
  ...
  return {
    ...
    saveNew: (options) ->
      ...
      return GoogleDriveApiService.newFile({
        name: options.name
        progress: progressListener
        data: data
        index: index
      ...
      .then (file) ->
        ...
        currentDriveFile = file[0]
        localStorage.setItem('data:' + file[0].id, JSON.stringify({file: file, data: data}))
        startUpdating()
        return Promise.resolve(file[0])

    loadFile: (options) ->
      progressListener = options.progress
      existingString = localStorage.getItem('data:' + options.id)
      try
      ...
      if existing and existing.file
        ...
        loadPromise = GoogleDriveApiService.loadFile({
        id: options.id,
        progress: options.progress
        })
      return loadPromise.then (result) ->
        currentDriveFile = result.file
