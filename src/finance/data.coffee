...
deserializePayment = (p) ->
  ...
  account = p.account
  switch p.type
    when Types.Simple then return new SimplePayment(account, date, eur(p.amount), p.description)
    when Types.Borrow then return new BorrowPayment(account, date, moment(p.returnDate, "D-M-YYYY"), eur(p.amount), p.description)
    when Types.SimpleExpense then return new SimplePayment(account, date, eur(-p.amount), p.description)
    when Types.TaxableIncome
      params =
        description: p.description
        earnedAt: date
        paymentDate: date
      return new TaxableIncomePayment(account, eur(p.amount), params)
